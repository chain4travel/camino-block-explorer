...
import { loadNumberOfTransactions, loadTotalGasFess } from 'store/cchainSlice/utils'
...
import { loadNumberOfPXTransactions, loadTotalPXGasFess } from 'store/xchainSlice/utils'
...
export default function RowRadioButtonsGroup({
    ...
    useEffect(() => {
        if (location.pathname.split('/')[1] === ChainType.C_CHAIN) dispatch(changetimeFrame(value))
    ...
    const { gasFeesLoading: pGasFeesLoading, transactionsLoading: pTransactionsLoading } =
        useAppSelector(getPchainOverreview)
    const { gasFeesLoading: xGasFeesLoading, transactionsLoading: xTransactionsLoading } =

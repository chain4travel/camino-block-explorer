...
const appConfigSlice = createSlice({
    ...
    reducers: {
        changeNetwork: (state, action) => {
            let active = state.networks.find(item => item.displayName === action.payload)
        ...
        removeCustomNetwork: (state, action) => {
            state.networks = state.networks.filter(item => item.id !== action.payload)
        },
        resetChains: state => {
            state.chains = []
        },
...
export const getActiveNetwork = (state: RootState) => state.appConfig.activeNetwork
...
export const { changeNetwork, addCustomNetwork, removeCustomNetwork, resetChains, changeTheme } =
